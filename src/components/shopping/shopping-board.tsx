...
import type { ShoppingItemView, ShoppingListView } from "@/lib/shopping-lists";
...
export function ShoppingBoard({ initialLists }: ShoppingBoardProps) {
  const [lists, setLists] = useState<ShoppingListView[]>(initialLists);
  ...
  async function handleCreateList(e: React.FormEvent) {
    ...
    try {
      ...
      const { list } = (await res.json()) as { list: ShoppingListView };
  ...
  async function handleCreateItem(e: React.FormEvent) {
    ...
    const optimisticItem: ShoppingItemView = { done: false, id: optimisticId, text };
  ...
  async function handleToggleItem(listId: string, itemId: string, currentDone: boolean) {
    ...
    try {
      const res = await fetch(`/api/shopping/items/${itemId}`, { method: "PATCH" });
  ...
  return (
    <div className="mx-auto w-full max-w-[1120px]">
      <h1 className="mb-5 font-serif text-2xl font-semibold text-[#171a18] sm:mb-6">Shopping</h1>
      <div className="grid items-start gap-5 xl:grid-cols-[340px_minmax(0,1fr)]">
        {/* Left pane: list of shopping lists */}
        <aside
        ...
        >
          {selectedList ? (
            ...
            <div className="flex flex-1 flex-col items-center justify-center py-20 text-center">
              <p className="font-serif text-lg text-[#5d635f]">Nothing on the list yet.</p>
